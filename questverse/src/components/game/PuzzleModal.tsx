...
import type { PuzzleHiddenObject, PuzzleSequence } from "@/types/scene";
...
function ModalShell({
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 px-4 backdrop-blur-sm">
      <div
        className={cn(
          "max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg",
      ...
      >
        <button
          type="button"
          onClick={onClose}
          className="float-right font-pixel text-xs text-[var(--color-text-muted)] hover:text-[var(--color-neon-magenta)]"
          aria-label="关闭谜题"
        >
          X
        </button>
