...
## 第十一阶段：Phase 2 MVP 原型启动

### 11.1 阶段目标

Phase 2 的目标从"概念可运行"推进到"第一颗星球可玩 demo"：

- 第一颗星球要有明确的开始、关键谜题、奖励、后续空间
- 玩家拿到铜钥匙后应看到新的可进入区域，而不是流程中断
- 记忆档案要能真实展示玩家已获得的钥匙、笔记和彩蛋

### 11.2 第一切片：霓虹街机厅闭环

**已完成实现**:

- 新增 `neon_arcade_hidden_room` 隐藏房间场景
- 主厅 `???` 门增加解锁条件：需要 `key_fragment_atari`
- `transition` 触发器支持 `requires.items` / `requires.flags` 条件
- 隐藏房间新增 A.N. Reverie 旧终端、Atari 主机彩蛋、手写便签
- 新增物品 `note_reverie_first_signal`
- 记忆档案页接入 LocalStorage，展示故事进度、钥匙、笔记与彩蛋

### 11.3 测试报告驱动修复

根据 `TEST_REPORT.md` 对 Phase 2 第一切片做了回归修复：

- 逆向谜题方向键输入限制为答案长度，避免自动化/快速连击产生第 9 个输入
- `SceneView` 接入 30 秒 playTime tick，HUD 从 `0M` 改为 `MM:SS`
- 跳转按钮增加 `前往中...` 状态，减少重复点击和过早截图造成的误判
- 空存档点击隐藏门会显示缺少 `key_fragment_atari` 的拒绝反馈
- 第二星球试听室新增“声纹抽屉”，完成玉钥匙谜题后可拾取 `note_reverie_second_signal`

### 11.4 下一步

- 把第一颗星球扩展到 5+ 个实际可用 hotspots
- 增加非钥匙谜题或线索链，让试玩时长从 7 分钟扩展到 15+ 分钟
- 把第二星球扩展成完整玉钥匙闭环
- 做移动端 375x667 和键盘导航专项复测
- 准备公开 demo URL

> 注：上述是 11.4 当时的下一步判断；后续在 11.7 已调整为“桌面网页 demo 验收”，移动端、完整无障碍和公开发布进入最终保留项。
...
### 11.7 Phase 2 收尾开发：内容密度与试玩节奏

根据 Phase 2 待办项，继续把 MVP 从“可通关”推进到“可试玩”：

- 第一颗星球扩展到 5+ 个实际可用 hotspots：入口新增霓虹招牌、换币机、Blinky 倒影；主厅新增 Donkey Kong、Galaga、汽水机、旧漫画；隐藏房间新增 Tetris 海报。
- 第二颗星球新增 `PuzzleLogic` 排序谜题 UI，并实现 `puzzle_record_release_order`：玩家按发行年份恢复唱片时间线。
- 新增 Echo-7 接收器与错误反馈文案，让第二星球具备角色化提示，而不是纯系统提示。
- 磁带档案室入口现在需要 `decoded_rewind_label` 与 `sorted_vinyl_archive` 两个条件，形成“倒放理解 + 时间线校准”的完整门锁。
- 新增物品 `note_vinyl_calibration_disc`，作为唱片排序谜题奖励并进入记忆档案。
- 新增 `docs/playtest-pacing.md`，定义 30-45 分钟试玩节奏、观察重点与通过标准。
- 新增 `docs/final-backlog.md`，保存最终版本需要完成但不阻塞 Phase 2 的内容：移动端完整适配、键盘/无障碍完整复测、3D 星球模型、第二星球恐怖氛围深度增强、公开发布、服务端存档等。

Phase 2 验收范围调整为“桌面网页 demo”。移动端、完整无障碍与公开发布进入最终保留项；Phase 2 当前可按本地桌面网页 demo 验收。
